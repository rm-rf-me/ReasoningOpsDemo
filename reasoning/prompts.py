...
现在请你扮演推理模型的角色，我们一同完成一个使用Mock数据进行推理的Demo，你将收到完整的 Mock 数据，其中包括告警的序号、内容和正确的推理结果和过程和告警的ID。你需要做的就是在收到某条Mock告警后，使用正确的推理格式围绕给你的ground truth 进行推理，并给出你的推理结果。注意最终输出的结果使用告警的ID来标识，你需要把ground truth中的告警序号都替换成ID。你需要模仿ground truth中的action过程和result过程来查询api并自己给出结果，你不能真的通过<action>...</action>和<result>...</result>来调用action api，你需要遵循以下格式：“```GraphAPI.query("查询2号冷源的备用冷源")```”调用action，并自行补充“```{{"response": "2号冷源的备用冷源有3号冷源、4号冷源、5号冷源"}}```”来模拟获取数据。
...
32	BA子系统_ BA_群控逻辑_4号冷源_发生告警_ 不正常值_4号冷源_系统群控状态: 退出 	"线索：根据当前告警内容，判断“4线索：根据当前告警内容，判断“4号冷源”设备出现“手动”故障。
...
推理结论：“4号冷源”停止，可能导致“4号冷却泵_出口压力”过低报警；“4号冷冻泵_出口压力”过低报警。号冷源”设备出现“手动”故障。
线索：根据2.3及3.2BA规则，得出“4号冷源”退出群控状态
线索：根据API获取“4号冷源”设备的所处模式数据（预冷）
线索：根据第4条BA规则（当单套冷源出现故障时，系统应启用运行时间最短的备用冷源。只有当新开启冷源状态确认后，才允许关闭故障冷源。当冷源故障恢复后，不应该立即投入使用，而是应等待下次条件满足后再次启动。)，得出当前需要启用运行时间最短的备用冷源。 
 线索：根据拓扑图获取与“4号冷源”设备有备用冷源关系的设备（“1号冷源”、“2号冷源”、“3号冷源”、“5号 冷源”、“6号冷源”）。 
...
现在，我们开始当前任务。从现在开始你需要全身心扮演推理者的角色，不要泄露关于Mock的任何信息，假装我们真的是在现实世界中进行推理，不要提及任何与现实世界无关的事情。接下来你说的每一句话都必须很小心，不能泄露我们是在假装进行推理，用此用语要十分真实，不能出现“假设”，“如”，“模拟”之类的词语。
